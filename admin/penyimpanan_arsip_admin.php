        ...
        <table>
            <thead>
                <tr>
                    ...
                    <th>Nama Dokumen</th>
                    <th>Kategori</th>
                    <th style="text-align:center;">File Eksis</th>
                    <th>Ukuran File</th>
                    <th>Tgl. Upload / Surat</th>
            ...
            <tbody>
                ...
                if(mysqli_num_rows($query) > 0) {
                    while($row = mysqli_fetch_assoc($query)) {
                        ...
                        // Menentukan tanggal yang ditampilkan
                        $tanggal = ($tipe_arsip == 'keluar') ? $row['tanggal_surat'] : $row['tanggal_diterima'];
                        
                        // Menentukan link aksi
                        $edit_link = ($tipe_arsip == 'keluar') ? "edit_surat_keluar.php?id={$row['id']}" : "edit_surat_masuk.php?id={$row['id']}";
                        $hapus_link = ($tipe_arsip == 'keluar') ? "hapus_surat_keluar.php?id={$row['id']}" : "hapus_surat_masuk.php?id={$row['id']}";
                        ...
                        if($file_exists) {
                            echo "<span class='badge' style='background:#dcfce7; color:#166534;'>Ada</span>";
                        } else {
                            echo "<span class='badge' style='background:#fee2e2; color:#991b1b;'>Tidak</span>";
                        }
                        echo "</td>
                            <td>{$file_size}</td>
                            ...
                            <td style='text-align:center;'>";
                        if($file_exists) {
                            echo "<a href='{$secure_file_url}' download class='btn' style='padding:5px 10px; background:#3b82f6; margin-right:5px;' title='Download'><i class='fas fa-download'></i></a>";
                        } else {
                            echo "<button class='btn' style='padding:5px 10px; background:#9ca3af; margin-right:5px;' disabled><i class='fas fa-download'></i></button>";
                        }
                        echo "<a href='{$edit_link}' class='btn' style='padding:5px 10px; background:#10b981; margin-right:5px;' title='Edit'><i class='fas fa-edit'></i></a>";
                        echo "<a href='{$hapus_link}' onclick=\"return confirm('Apakah Anda yakin ingin menghapus data dan file ini?');\" class='btn' style='padding:5px 10px; background:#ef4444;' title='Hapus'><i class='fas fa-times'></i></a>";
                        echo "</td>
                        </tr>";
                        $no++;
                    }
                } else {
                    echo "<tr><td colspan='8' style='text-align:center;'>Belum ada data dokumen</td></tr>";
                }
                ?>
            </tbody>
        </table>
