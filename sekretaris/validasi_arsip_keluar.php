...
if($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if($action == 'approve') {
        $q = "UPDATE surat_keluar SET status = 'Divalidasi', catatan_validasi = '' WHERE id = '$id_surat'";
        if(mysqli_query($conn, $q)) {
            $_SESSION['success'] = "Arsip keluar berhasil divalidasi.";
        }
    } else if($action == 'reject') {
        $q = "UPDATE surat_keluar SET status = 'Dikembalikan', catatan_validasi = '$catatan' WHERE id = '$id_surat'";
        if(mysqli_query($conn, $q)) {
            $_SESSION['success'] = "Arsip keluar dikembalikan dengan catatan.";
        }
    }
    echo "<script>window.location.href='validasi_arsip_keluar.php';</script>";
    exit();
}
...
<div class="header-action">
    <h2 class="page-title" style="margin-bottom:0;">Validasi Arsip Keluar</h2>
</div>
...
<div class="card">
    <p style="margin-bottom: 15px; color: #666;">Daftar arsip keluar yang perlu diperiksa kelengkapannya sebelum divalidasi.</p>
    <div style="overflow-x:auto;">
        <table>
            ...
            <tbody>
                <?php
                $no = 1;
                $query = mysqli_query($conn, "SELECT * FROM surat_keluar WHERE status = 'Pending' OR status = '' OR status IS NULL ORDER BY id DESC");
                if(mysqli_num_rows($query) > 0) {
                    while($row = mysqli_fetch_assoc($query)) {
                        $file_path = "../uploads/surat_keluar/" . $row['file_surat'];
                    ...
                    echo "<tr><td colspan='7' style='text-align:center;'>Tidak ada arsip keluar yang perlu divalidasi.</td></tr>";
                }
                ?>
            </tbody>
        </table>
